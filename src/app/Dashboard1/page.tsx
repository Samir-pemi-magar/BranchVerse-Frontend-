...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAllStories = async () => {
      try {
        ...
        setStories(all);
        setDiscoverStories(shuffled.slice(0, 3));
      } catch (err) {
  ...
  useEffect(() => {
    if (!statsInView || hasAnimatedStats.current) return;
    ...
    >
          ...
          >
            {stories.length === 0 ? (
              ...
              stories.map((story) => (
                <div
                  key={story._id}
                  className="relative flex-shrink-0 min-w-full h-[260px] sm:h-[300px] rounded-2xl overflow-hidden snap-center cursor-pointer group"
                >
                  <img
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-10xl mx-auto">
          {discoverStories.length === 0 ? (
            ...
            discoverStories.map((story) => (
              <div
                key={story._id}
                className="relative w-full h-[220px] sm:h-[250px] rounded-2xl overflow-hidden cursor-pointer group"
              >
                <img
                  src={coverUrl(story.cover)}
                  alt={story.title}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-12 md:gap-6 justify-center items-center md:items-start max-w-5xl mx-auto">
          {[
            {
              num: "1",
              label: "Start a Story",
              desc: "Initiate your narrative with a compelling premise and establish the universe.",
              svg: (
                <svg
                  width="62"
                  height="50"
            ...
            },
            {
              num: "2",
              label: "Branch & Expand",
              desc: "Invite others or contribute to existing stories by adding new branches and plot developments.",
              svg: (
                <svg
                  width="54"
                  height="50"
            ...
            },
            {
              ...
              desc: "Navigate through branching paths, discover diverse outcomes, and connect with fellow creators.",
              svg: (
                <svg
                  width="54"
                  height="49"
          ...
          ].map(({ num, label, desc, svg }) => (
            <div
              key={num}
              className="flex flex-col items-center max-w-[280px] text-center"
            >
              <div className="rounded-full h-[54px] w-[54px] flex items-center justify-center bg-[#00B8AE] shrink-0">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-10 sm:gap-16 md:gap-24 items-center">
          {[
            { value: animatedWriters, label: "Writers" },
            { value: animatedStories, label: "Stories" },
            { value: animatedReaders, label: "Readers" },
          ].map(({ value, label }) => (
